...
This repo was built by 7 parallel agents against a shared contract. `agent-1-foundation`
(merged into `main`) is the foundation: project scaffold, design system, routing contract,
...
`src/pages/{auth,marketing,screening,scans,patients,admin}/*` are all real, merged from their
respective agent branches. **`src/pages/review/routes.tsx` (Doctor Review & Referrals) is still
the original placeholder stub** — no `agent-5-*` branch was ever built for this workstream, so
`/referrals` currently renders a "coming soon" placeholder and `reviewNavItems` is empty. This is
the one gap left in the 7-workstream plan; whoever picks it up should replace this file with a
real `reviewRoutes`/`reviewNavItems` pair matching the same `RouteObject[]`/`NavItem[]` contract
every other feature folder uses, and can wire it against `ROUTES.referrals` /
`ROUTES.referralDetail`, which already exist in `src/lib/routes.ts`.
